...
::ComputeData()
{
	...
	{
		thresholdFilter->SetUpperThreshold( m_BrightLine );
		thresholdFilter->SetLowerThreshold( m_BrightLine );
	}
	else
	{
...
//	vesselnessFilter->SetInput( eigenValuesArrayFilter->GetOutput() );
...
::RealType LineShapeImageFilter< TInputImage, TOutputImage, TLabelImage, TVectorImage >
::VesselnessMeasure
::operator()( const EigenValuesArrayType & A ) const
{
	if( A[2] > 0 && A[1] > 0 )
	return std::exp( - ( std::abs(A[2])-std::abs(A[1])) * std::abs(A[0])*std::abs(A[2]) * std::sqrt( A[0]*A[0]+A[1]*A[1]+A[2]*A[2] ) / ( (std::abs(A[2])-std::abs(A[0]))*(std::abs(A[1])-std::abs(A[0]))*(std::abs(A[2])-std::abs(A[0]))*(std::abs(A[1])-std::abs(A[0])) ) )
		* std::exp( - (std::abs(A[2])-std::abs(A[1])) * (std::abs(A[2])-std::abs(A[1])) / ( (std::abs(A[1])-std::abs(A[0]))*(std::abs(A[1])-std::abs(A[0])) ) );
	return 0;
}
